...
<html lang="en">
    ...
    ?>
        ...
        <div class="rules">
            ...
            <form id="reservationForm" method="post" enctype="multipart/form-data" action="<?=ROOT?>/external/cricketturfform/reserve">
            <input type="hidden" id="description" name="description" value="no">
            <input type="hidden" name="court_name" value="cricketturf">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value) {
                ...
                formData.append('court_name', 'cricketturf'); // Use the actual court name
                ...
                xhr.open('POST', '<?=ROOT?>/external/cricketturfform/getPrice', true);
        ...
        function updateDiscountedPrice() {
            ...
            if (priceInput.value && userTypeSelect.value) {
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '<?=ROOT?>/external/cricketturfform/getDiscount', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    // Show the appropriate container based on selection
     if (durationSelect.value === '2 hour') {
...
function checkAvailability() {
    ...
    formData.append('court_name', 'cricketturf');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/cricketturfform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/cricketturfform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
