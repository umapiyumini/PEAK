...
<h1 class="title1">Table Tennis court Reservation</h1>
...
<form id="reservationForm" method="post" action="<?=ROOT?>/external/tabletennisform/reserve" enctype="multipart/form-data">
    <!-- Booking For -->
    <input type="hidden" name="court_name" value="table tennis">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value ) {
                ...
                formData.append('court_name', 'table tennis'); // Use the actual court name
                ...
                xhr.open('POST', '<?=ROOT?>/external/tabletennisform/getPrice', true);
        ...
        function updateDiscountedPrice() {
            ...
            if (priceInput.value && userTypeSelect.value) {
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '<?=ROOT?>/external/tabletennisform/getDiscount', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkAvailability() {
    ...
    formData.append('court_name', 'table tennis');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/tabletennisform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/tabletennisform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
