...
<html lang="en">
    ...
    ?>
        ...
        <h1 class="title1">Cricket court Reservation</h1>
        ...
        <div class="rules">
            ...
            <form id="reservationForm" method="post" enctype="multipart/form-data" action="<?=ROOT?>/external/cricketform/reserve">
            <input type="hidden" name="court_name" value="cricket">
<!-- Booking For -->
...
<div id="halfDayOptions" style="display:none;">
    ...
    <div class="slot-container">
        ...
        <label><input type="radio" name="time_slot" id="slot2" value="14:00:00"> 14:00 - 18:00</label>
    </div>
</div>
...
<!-- Duration -->
<label for="description">Softball or hardball:</label>
<select id="description" name="description" required>
<option   selected disables>Type</option>
    <option   value="Hard ball">Hard ball</option>
    <option value="Softball">Soft ball</option>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value && descriptionSelect.value) {
                ...
                formData.append('court_name', 'cricket'); // Use the actual court name
                ...
                xhr.open('POST', '<?=ROOT?>/external/cricketform/getPrice', true);
        ...
        function updateDiscountedPrice() {
            ...
            if (priceInput.value && userTypeSelect.value) {
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '<?=ROOT?>/external/cricketform/getDiscount', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkAvailability() {
    ...
    formData.append('court_name', 'cricket');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/cricketform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/cricketform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
