...
<h1 class="title1">Sportcenter Reservation</h1>
...
<form id="reservationForm" method="post" action="<?=ROOT?>/external/sportscenterform/reserve" enctype="multipart/form-data">
    <!-- Booking For -->
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value ) {
                ...
                formData.append('court_name', 'sportscenter'); // Use the actual court name
                ...
                xhr.open('POST', '<?=ROOT?>/external/sportscenterform/getPrice', true);
        ...
        function updateDiscountedPrice() {
            ...
            if (priceInput.value && userTypeSelect.value) {
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '<?=ROOT?>/external/sportscenterform/getDiscount', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkAvailability() {
    ...
    formData.append('court_name', 'sportscenter');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/sportscenterform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/sportscenterform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
