...
<?php
    ...
    <h1 class="title1">Hockey court Reservation</h1>
    ...
    <form id="reservationForm" method="post" enctype="multipart/form-data" action="<?=ROOT?>/external/hockeyform/reserve">


    <input type="hidden" name="court_name" value="hockey">
        <!-- Booking For -->
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value && descriptionSelect.value) {
                ...
                xhr.open('POST', '<?=ROOT?>/external/hockeyform/getPrice', true);
...
function checkAvailability() {
    ...
    formData.append('court_name', 'hockey');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/hockeyform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/hockeyform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
