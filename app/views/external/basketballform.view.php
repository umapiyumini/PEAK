...
<html lang="en">
    ...
    ?>
        ...
        <h1 class="title1">Basketball court Reservation</h1>
        ...
        <div class="rules">
            ...
            <form id="reservationForm" method="post" enctype="multipart/form-data" action="<?=ROOT?>/external/basketballform/reserve">

            <input type="hidden" name="court_name" value="basketball">
<!-- Booking For -->
...
<!-- Duration -->
<label for="description">Do you want a light?</label>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        userTypeSelect.addEventListener('change',updateDiscountedPrice);
         ...
         function updatePrice() {
            ...
            if (bookingForSelect.value && durationSelect.value && descriptionSelect.value) {
                ...
                formData.append('court_name', 'basketball'); // Use the actual court name
                ...
                xhr.open('POST', '<?=ROOT?>/external/basketballform/getPrice', true);
        ...
        function updateDiscountedPrice() {
            ...
            if (priceInput.value && userTypeSelect.value) {
                ...
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '<?=ROOT?>/external/basketballform/getDiscount', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkAvailability() {
    ...
    formData.append('court_name', 'basketball');
    
    // Send request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/basketballform/checkFullDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
function checkHalfDayAvailability(date) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '<?=ROOT?>/external/basketballform/checkHalfDayAvailability', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
