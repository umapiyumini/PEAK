...
function addPlayerToRoster(regNo, position, number) {
    ...
    row.innerHTML = `
        <td><a href="studentProfile.html?regNo=${regNo}" class="profile-link">${regNo}</a></td>
        <td>${position}</td>
        <td>${number}</td>
...
form.addEventListener("submit", function (e) {
    ...
    if (editingRow) {
        ...
        editingRow.cells[0].innerHTML = `<a href="studentProfile.html?regNo=${editingRow.getAttribute("data-regno")}" class="profile-link">${regNo}</a>`;
